    ...
    def _get_transforms(mode):
        if mode == 'train':
            return A.Compose([
                ...
                A.CoarseDropout(                 # ✂️ ГРУБОЕ ВЫРЕЗАНИЕ ОБЛАСТЕЙ
                    max_holes=8,                 # Максимум 8 вырезанных областей
                    max_height=32,               # Максимальная высота области: 32px
                    max_width=32,                # Максимальная ширина области: 32px
                    p=0.2                        # 20% вероятность - помогает против переобучения
                ),
